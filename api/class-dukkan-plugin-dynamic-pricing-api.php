...
class Dukkan_Plugin_Dynamic_Pricing_API {
	...
	private static $VALID_METHODS = array( 'simple', 'bulk', 'bogo', 'bogo_repeat' );
	...
	// --- Pricing methods for BOGO rules ---
	private static $BOGO_PRICING_METHODS = array(
		'discount__percentage',
		'discount__amount',
		'fixed__price',
	);
	...
	// --- Condition type keys accepted by the API in the conditions array ---
	private static $CONDITION_TYPES = array( 'cart__quantity', 'cart__coupons', 'product__attributes', 'product__tags', 'product__variation', 'product__product', 'product__category' );
	...
	// --- Maps condition type to the field key WCDPD stores its value under ---
	...
	public function create_rule( $request ) {
		...
		if ( ! $has_products && ! $has_cats && ! $has_attributes && ! $has_tags && ! $has_variations ) {
			// For BOGO, also check for product__product / product__category in conditions
			$has_cond_product = false;
			$has_cond_cat     = false;
			if ( is_array( $conditions ) ) {
				foreach ( $conditions as $c ) {
					$t = $c['type'] ?? '';
					if ( 'product__product' === $t )  $has_cond_product = true;
					if ( 'product__category' === $t ) $has_cond_cat     = true;
				}
			}
			if ( ! $has_cond_product && ! $has_cond_cat ) {
				return new WP_Error( 'missing_filter',
					'At least one product filter is required: product_uids, product_category_uids, or a condition with product__product, product__category, product__attributes, product__tags, or product__variation.',
					array( 'status' => 400 ) );
			}
		}
		...
		if ( 'simple' === $rule_method ) {
		...
		} elseif ( in_array( $rule_method, array( 'bogo', 'bogo_repeat' ), true ) ) {
			// BOGO: requires quantities_based_on, purchase/receive quantities, bogo pricing
			$qty_based_on        = sanitize_text_field( $body['quantities_based_on'] ?? '' );
			$purchase_qty        = $body['bogo_purchase_quantity'] ?? null;
			$receive_qty         = $body['bogo_receive_quantity'] ?? null;
			$bogo_pricing_method = sanitize_text_field( $body['bogo_pricing_method'] ?? '' );
			$bogo_pricing_value  = $body['bogo_pricing_value'] ?? null;
			$bogo_product_conds  = $body['bogo_product_conditions'] ?? null;
			...
			if ( ! is_numeric( $purchase_qty ) || (int) $purchase_qty < 1 ) {
				return new WP_Error( 'invalid_bogo_purchase_quantity',
					'bogo_purchase_quantity must be an integer >= 1.', array( 'status' => 400 ) );
			}
			if ( ! is_numeric( $receive_qty ) || (int) $receive_qty < 1 ) {
				return new WP_Error( 'invalid_bogo_receive_quantity',
					'bogo_receive_quantity must be an integer >= 1.', array( 'status' => 400 ) );
			}
			if ( ! in_array( $bogo_pricing_method, self::$BOGO_PRICING_METHODS, true ) ) {
				return new WP_Error( 'invalid_bogo_pricing_method',
					'bogo_pricing_method must be one of: ' . implode( ', ', self::$BOGO_PRICING_METHODS ),
					array( 'status' => 400 ) );
			}
			if ( ! is_numeric( $bogo_pricing_value ) || $bogo_pricing_value < 0 ) {
				return new WP_Error( 'invalid_bogo_pricing_value',
					'bogo_pricing_value must be >= 0.', array( 'status' => 400 ) );
			}

			$rule['quantities_based_on']    = $qty_based_on;
			$rule['bogo_purchase_quantity'] = (int) $purchase_qty;
			$rule['bogo_receive_quantity']  = (int) $receive_qty;
			$rule['bogo_pricing_method']    = $bogo_pricing_method;
			$rule['bogo_pricing_value']     = (float) $bogo_pricing_value;

			// Validate and store bogo_product_conditions (optional for XX, required for XY)
			$rule['bogo_product_conditions'] = array();
			if ( is_array( $bogo_product_conds ) ) {
				foreach ( $bogo_product_conds as $i => $gc ) {
					$err = $this->validate_cart_condition( $gc, $i );
					if ( is_wp_error( $err ) ) return $err;
					$rule['bogo_product_conditions'][] = $this->build_cart_condition( $gc );
				}
			}
	...
	public function update_rule( $request ) {
		...
		// --- Bulk: quantity_ranges (bulk only, not BOGO) ---
		if ( ! $is_simple && ! in_array( $old['method'] ?? '', array( 'bogo', 'bogo_repeat' ), true ) && isset( $body['quantity_ranges'] ) ) {
		...
		// --- BOGO: bogo_purchase_quantity / bogo_receive_quantity ---
		if ( in_array( $old['method'] ?? '', array( 'bogo', 'bogo_repeat' ), true ) ) {
			if ( isset( $body['bogo_purchase_quantity'] ) ) {
				$pq = $body['bogo_purchase_quantity'];
				if ( ! is_numeric( $pq ) || (int) $pq < 1 ) {
					return new WP_Error( 'invalid_bogo_purchase_quantity',
						'bogo_purchase_quantity must be an integer >= 1.', array( 'status' => 400 ) );
				}
				$old['bogo_purchase_quantity'] = (int) $pq;
			}
			if ( isset( $body['bogo_receive_quantity'] ) ) {
				$rq = $body['bogo_receive_quantity'];
				if ( ! is_numeric( $rq ) || (int) $rq < 1 ) {
					return new WP_Error( 'invalid_bogo_receive_quantity',
						'bogo_receive_quantity must be an integer >= 1.', array( 'status' => 400 ) );
				}
				$old['bogo_receive_quantity'] = (int) $rq;
			}
			if ( isset( $body['bogo_pricing_method'] ) ) {
				$bpm = sanitize_text_field( $body['bogo_pricing_method'] );
				if ( ! in_array( $bpm, self::$BOGO_PRICING_METHODS, true ) ) {
					return new WP_Error( 'invalid_bogo_pricing_method',
						'bogo_pricing_method must be one of: ' . implode( ', ', self::$BOGO_PRICING_METHODS ),
						array( 'status' => 400 ) );
				}
				$old['bogo_pricing_method'] = $bpm;
			}
			if ( isset( $body['bogo_pricing_value'] ) ) {
				$bpv = $body['bogo_pricing_value'];
				if ( ! is_numeric( $bpv ) || $bpv < 0 ) {
					return new WP_Error( 'invalid_bogo_pricing_value',
						'bogo_pricing_value must be >= 0.', array( 'status' => 400 ) );
				}
				$old['bogo_pricing_value'] = (float) $bpv;
			}
			// bogo_product_conditions
			if ( isset( $body['bogo_product_conditions'] ) ) {
				$bgc = $body['bogo_product_conditions'];
				if ( ! is_array( $bgc ) ) {
					return new WP_Error( 'invalid_bogo_product_conditions',
						'bogo_product_conditions must be an array.', array( 'status' => 400 ) );
				}
				$validated = array();
				foreach ( $bgc as $i => $gc ) {
					$err = $this->validate_cart_condition( $gc, $i );
					if ( is_wp_error( $err ) ) return $err;
					$validated[] = $this->build_cart_condition( $gc );
				}
				$old['bogo_product_conditions'] = $validated;
			}
		}
	...
	private function to_response( $rule ) {
		...
		foreach ( ( $rule['conditions'] ?? array() ) as $c ) {
			...
			} elseif ( 'product__attributes' === $t ) {
				$conds[] = array(
					'type'               => $t,
					'method_option'      => $c['method_option'] ?? '',
					'product_attributes' => isset( $c['product_attributes'] ) ? array_map( 'intval', $c['product_attributes'] ) : array(),
				);
			} elseif ( 'product__tags' === $t ) {
				$conds[] = array(
					'type'          => $t,
					'method_option' => $c['method_option'] ?? '',
		...
		} elseif ( in_array( $method, array( 'bogo', 'bogo_repeat' ), true ) ) {
			// BOGO fields
			$resp['quantities_based_on']    = $rule['quantities_based_on'] ?? '';
			$resp['bogo_purchase_quantity'] = (int) ( $rule['bogo_purchase_quantity'] ?? 0 );
			$resp['bogo_receive_quantity']  = (int) ( $rule['bogo_receive_quantity'] ?? 0 );
			$resp['bogo_pricing_method']    = $rule['bogo_pricing_method'] ?? '';
			$resp['bogo_pricing_value']     = (float) ( $rule['bogo_pricing_value'] ?? 0 );
			// Format bogo_product_conditions
			$bogo_conds = array();
			foreach ( ( $rule['bogo_product_conditions'] ?? array() ) as $bc ) {
				$bogo_conds[] = $this->condition_to_response( $bc );
			}
			$resp['bogo_product_conditions'] = $bogo_conds;
		} else {
	...
	// =====================================================================
	// SINGLE CONDITION TO RESPONSE HELPER
	// =====================================================================

	private function condition_to_response( $c ) {
		$t = $c['type'] ?? '';
		if ( 'cart__quantity' === $t ) {
			return array(
				'type'          => $t,
		...
		} elseif ( 'product__product' === $t ) {
			return array(
				'type'          => $t,
				'method_option' => $c['method_option'] ?? '',
				'products'      => isset( $c['products'] ) ? array_map( 'intval', $c['products'] ) : array(),
			);
		} elseif ( 'product__category' === $t ) {
			return array(
				'type'               => $t,
				'method_option'      => $c['method_option'] ?? '',
				'product_categories' => isset( $c['product_categories'] ) ? array_map( 'intval', $c['product_categories'] ) : array(),
			);
		}
		return $c;
	}
	...
	private function validate_cart_condition( $c, $i ) {
		...
		if ( 'product__attributes' === $c['type'] ) {
			if ( ! isset( $c['method_option'] ) || ! in_array( $c['method_option'], self::$LIST_ADVANCED_METHODS, true ) ) {
				return new WP_Error( 'invalid_condition_method',
					$label . ': method_option must be one of: ' . implode( ', ', self::$LIST_ADVANCED_METHODS ),
					array( 'status' => 400 ) );
			}
			if ( ! isset( $c['product_attributes'] ) || ! is_array( $c['product_attributes'] ) || empty( $c['product_attributes'] ) ) {
				return new WP_Error( 'invalid_condition_attributes',
					$label . ': product_attributes must be a non-empty array of attribute term IDs.',
					array( 'status' => 400 ) );
			}
		}
		...
		if ( 'product__tags' === $c['type'] ) {
			...
			if ( ! isset( $c['product_tags'] ) || ! is_array( $c['product_tags'] ) || empty( $c['product_tags'] ) ) {
				return new WP_Error( 'invalid_condition_tags',
					$label . ': product_tags must be a non-empty array of product tag term IDs.',
					array( 'status' => 400 ) );
			}
		}
		...
		if ( 'product__product' === $c['type'] ) {
			if ( ! isset( $c['method_option'] ) || ! in_array( $c['method_option'], self::$LIST_METHODS, true ) ) {
				return new WP_Error( 'invalid_condition_method',
					$label . ': method_option must be one of: ' . implode( ', ', self::$LIST_METHODS ),
					array( 'status' => 400 ) );
			}
			if ( ! isset( $c['products'] ) || ! is_array( $c['products'] ) || empty( $c['products'] ) ) {
				return new WP_Error( 'invalid_condition_products',
					$label . ': products must be a non-empty array of product IDs.',
					array( 'status' => 400 ) );
			}
			foreach ( $c['products'] as $pid ) {
				if ( ! wc_get_product( $pid ) ) {
					return new WP_Error( 'invalid_product',
						$label . ": Product ID $pid does not exist.", array( 'status' => 400 ) );
				}
			}
		}

		if ( 'product__category' === $c['type'] ) {
			if ( ! isset( $c['method_option'] ) || ! in_array( $c['method_option'], self::$LIST_METHODS, true ) ) {
				return new WP_Error( 'invalid_condition_method',
					$label . ': method_option must be one of: ' . implode( ', ', self::$LIST_METHODS ),
					array( 'status' => 400 ) );
			}
			if ( ! isset( $c['product_categories'] ) || ! is_array( $c['product_categories'] ) || empty( $c['product_categories'] ) ) {
				return new WP_Error( 'invalid_condition_categories',
					$label . ': product_categories must be a non-empty array of category IDs.',
					array( 'status' => 400 ) );
			}
			foreach ( $c['product_categories'] as $cid ) {
				$term = get_term( $cid, 'product_cat' );
				if ( ! $term || is_wp_error( $term ) ) {
					return new WP_Error( 'invalid_category',
						$label . ": Category ID $cid does not exist.", array( 'status' => 400 ) );
				}
			}
		}
	...
	private function build_cart_condition( $c ) {
		...
		if ( 'product__attributes' === $c['type'] ) {
			$cond['product_attributes'] = array_map( 'strval', $c['product_attributes'] );
		}
		...
		if ( 'product__product' === $c['type'] ) {
			$cond['products'] = array_map( 'strval', $c['products'] );
		}

		if ( 'product__category' === $c['type'] ) {
			$cond['product_categories'] = array_map( 'strval', $c['product_categories'] );
		}
